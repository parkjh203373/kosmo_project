                ...
                <div class="container py-4">
					    ...
					    <div class="col-md-7">
					        <form action="./myboard" method="get" class="input-group bg-white shadow-sm d-flex align-items-center" 
					            ...
					            <input type="text" name="search" 
					                   class="form-control border-0 bg-transparent pl-3 fixed-action-input" 
					                   placeholder="원하는 도서 제목이나 저자 키워드를 입력해 보세요." 
					                   value="${param.search}"
					                   style="box-shadow: none !important; flex: 1;">
					            
					            <div class="input-group-append" style="margin-left: 6px;">
					                <button class="btn btn-primary px-4 font-weight-bold d-flex align-items-center justify-content-center" 
					                        style="border-radius: 10px; height: 42px; min-width: 80px;" 
					                        type="submit">
					                    검색
					                </button>
					            </div>
                    ...
                    <div class="row">
                        <c:forEach items="${list}" var="dto">
                            <div class="col-xl-custom-5 col-md-6 mb-4">
								...
								<ul class="pagination pagination-sm border-0">
									...
									<c:if test="${pager.pre}">
									    <li class="page-item">
									        <a class="page-link border-0 text-dark bg-light mx-1 rounded-circle text-center"
									           ...
									           href="./myboard?page=${pager.start - 1}&search=${pager.search}">
									...
									<c:forEach begin="${pager.start}" end="${pager.end}" var="i">
										<li class="page-item mx-1 ${pager.page == i ? 'active' : ''}">
											...
											href="./myboard?page=${i}&search=${pager.search}">${i}</a>
										</li>
									</c:forEach>
									...
									<c:if test="${pager.next}">
									    <li class="page-item">
									        <a class="page-link border-0 text-dark bg-light mx-1 rounded-circle text-center"
									           style="width: 34px; height: 34px; line-height: 20px;"
									           href="./myboard?page=${pager.end + 1}&search=${pager.search}">
							...
							<div class="col-12 text-center py-5">
								<c:choose>
									...
									<c:otherwise>
										...
										<p class="text-gray-500">작성하신 글 목록이 없습니다.</p>
									</c:otherwise>
								</c:choose>
							</div>
						</c:if>
