...
<div id="wrapper">
		...
		<div id="content-wrapper" class="d-flex flex-column">
		    ...
		    <div class="col-lg-6 pr-lg-4 border-right d-flex flex-column">
		        <div class="d-flex align-items-center justify-content-between mb-4" style="height: 38px;">
		            <h2 class="h5 mb-0 text-gray-900 font-weight-bold d-flex align-items-center">
		        ...
		        <div class="row flex-grow-1 align-items-stretch">
		            <c:forEach items="${bestSeller}" var="best" varStatus="vs">
		                <div class="col-4 mb-3 text-center d-flex flex-column align-items-center">
		                    <div class="position-relative mb-2 shadow rounded-lg overflow-hidden" style="height: 200px; width: 140px; min-width: 140px;">
		                        <a href="./detail?bookNum=${best.bookNum}" class="d-block h-100 w-100">
		                            <img src="${best.bookImage}" class="h-100 w-100" 
		                                 style="object-fit: cover;" 
		                                 onerror="this.src='/img/no-image.png'" alt="도서 이미지">
		                        </a>
		                        <div class="position-absolute" style="top: 8px; left: 8px; z-index: 2;">
		                            <c:choose>
		                                <c:when test="${vs.count eq 1}"><span class="badge shadow-sm font-weight-bold text-white px-2 py-1" style="background:#e5a93b; border-radius: 6px; font-size: 0.75rem;">1st</span></c:when>
		                                <c:when test="${vs.count eq 2}"><span class="badge shadow-sm font-weight-bold text-white px-2 py-1" style="background:#a0aec0; border-radius: 6px; font-size: 0.75rem;">2nd</span></c:when>
		                                <c:otherwise><span class="badge shadow-sm font-weight-bold text-white px-2 py-1" style="background:#b7791f; border-radius: 6px; font-size: 0.75rem;">3rd</span></c:otherwise>
		                            </c:choose>
		                        </div>
		                    </div>
		                    <div class="w-100 px-1 text-center" style="height: 42px;">
		                        <div class="small font-weight-bold text-gray-800 text-truncate mb-0" title="${best.bookTitle}">
		                            ${best.bookTitle}
		                        </div>
		                        <div class="text-xs text-muted text-truncate mt-0.5">
						            ${best.bookAuthor}
						        </div>
		                        <div class="text-xs text-muted mt-0.5"><i class="fas fa-heart text-danger mr-1"></i> ${best.rentCount}회 대출</div>
		                    </div>
		                </div>
		            </c:forEach>
		        </div>
		    ...
		    <div class="col-lg-6 pl-lg-4 d-flex flex-column">
		        <div class="d-flex align-items-center justify-content-between mb-4" style="height: 38px;">
		            <h2 class="h5 mb-0 text-gray-900 font-weight-bold d-flex align-items-center">
		                <i class="fas fa-users" style="color: #2a5c43 !important; margin-right: 8px;"></i>독자 연령대별 선호 도서
		            </h2>
		            <select id="ageSelector" class="form-control form-control-sm border shadow-sm bg-white" style="width: 120px; font-size: 0.85rem; height: 35px; padding-top: 0; padding-bottom: 0; border-color: #cbd5e0 !important; border-radius: 8px;">
		        ...
		        <div id="ageBestResult" class="row flex-grow-1 align-items-stretch">
							    ...
							    <div class="card-body p-1">
							        <form action="./list" method="get" class="form-inline row">
							            ...
							            <div class="col-md-7 mb-2 mb-md-0">
							            	<input type="text" name="search" value="${pager.search}" class="form-control w-100 h-50 border-0 bg-light" placeholder="원하는 도서 제목이나 저자 키워드를 입력해보세요.">
							            </div>
							            <div class="col-md-2">
								        ...
								        <c:forEach items="${list}" var="d">
								            <tr class="book-hover-row" style="border-bottom: 1px solid #edf2f7;">
								                ...
								                <td class="align-middle py-3">
								                    ...
								                    <div class="text-muted small">저자 : ${d.bookAuthor} / 출판사 : ${d.bookPublisher}</div>
								                </td>
